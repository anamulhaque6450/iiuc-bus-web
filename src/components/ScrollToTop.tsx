      ...
      {isVisible && (
        <div className="fixed bottom-6 right-6 z-50">
          ...
          <button
            ...
            className={`group relative bg-gradient-to-r from-orange-500 via-red-500 to-pink-500 hover:from-orange-600 hover:via-red-600 hover:to-pink-600 text-white p-4 rounded-full shadow-2xl hover:shadow-orange-500/25 transition-all duration-300 border-2 border-white/20 backdrop-blur-sm ${
              isLaunching 
                ? 'animate-rocket-launch cursor-not-allowed' 
                : 'hover:scale-110 animate-bounce-in hover:animate-rocket-hover'
            }`}
            aria-label="Launch rocket to top"
          >
            <div className="relative">
              {/* Rocket Icon */}
              <Rocket className={`h-6 w-6 relative z-10 transition-transform duration-300 ${
                isLaunching ? 'rotate-0' : 'group-hover:rotate-12'
              }`} />
              
              {/* Fire/Exhaust Effects */}
              <div className={`absolute top-full left-1/2 transform -translate-x-1/2 transition-all duration-300 ${
              ...
              {/* Tooltip */}
              <div className={`absolute bottom-full right-0 mb-3 px-3 py-2 bg-gray-900 text-white text-sm rounded-lg transition-all duration-200 whitespace-nowrap ${
                isLaunching ? 'opacity-0' : 'opacity-0 group-hover:opacity-100'
              }`}>
                🚀 Launch to Top!
          ...
          {isLaunching && (
            <div className="absolute bottom-full right-0 mb-4 px-4 py-2 bg-green-500 text-white text-sm rounded-lg animate-fade-slide-up shadow-lg">
              🚀 Launching to space! 
            </div>
          )}
        </div>
